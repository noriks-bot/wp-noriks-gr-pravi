...
function gck_gr_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $noun = ( $n === 1 ) ? 'νάρθηκα' : 'νάρθηκες';
...
function gck_render_bundle_selector() {
    ...
    if ( has_term( array( 'orto-cloath' ), 'product_cat', $product_id ) ) {
        $gck_offer_subs = array( 'Σετ για το μπάνιο', 'Πακέτο για όλο το σπίτι', 'Πακέτο για οικογένεια και φίλους' );
    } else {
        $gck_offer_subs = array();
    }
    $gck_offer_badges = has_term( array( 'orto-cloath', 'orto-cloud' ), 'product_cat', $product_id )
        ? array( 1 => 'ΚΟΡΥΦΑΙΑ ΕΠΙΛΟΓΗ', 2 => 'ΚΑΛΥΤΕΡΗ ΤΙΜΗ' )
        : array();
    ...
    // attributes to a single Boja/Velicina, so the 3 groups' colour/size options
    // are sourced from the product's real attributes with hardcoded fallbacks,
    // fed into the EXACT SHBOX swatch/select markup + CSS (see the render branch
    // below). Detected early and bypasses the attribute guards so it always
    // renders regardless of product attributes.
    ...
    <?php
    // SHGIFTS: the carapa size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <?php if ( $show_countdown ) : ?>
        ...
        <div class="gck-countdown" id="gck-countdown"
            ...
            <div class="gck-countdown__body">
                Η προσφορά ισχύει μόνο σήμερα — ακόμη <span class="gck-countdown__timer" aria-live="polite">--:--</span>.<?php
                if ( $countdown_stock > 0 ) :
                    ?> Έμειναν μόνο <strong><?php echo esc_html( $countdown_stock ); ?> τεμάχια</strong>.<?php
                endif; ?> <strong>Βιαστείτε πριν εξαντληθούν!</strong>
            </div>
        </div>
        <script>
          (function(){
    ...
    <style>
      ...
      /* lokalizirane barve GR */
      #bundle-selector .swatch-circle.color-\%ce\%bc\%cf\%80\%ce\%bb\%ce\%b5 { background: #b7c4e6 !important; }
      #bundle-selector .swatch-circle.color-\%cf\%81\%ce\%bf\%ce\%b6 { background: #e6a8b8 !important; }
      #bundle-selector .swatch-circle.color-\%ce\%b3\%ce\%ba\%cf\%81\%ce\%b9 { background: #c9c2ba !important; }
      #bundle-selector .swatch-circle.color-\%cf\%80\%cf\%81\%ce\%ac\%cf\%83\%ce\%b9\%ce\%bd\%ce\%bf { background: #b3bf9a !important; }
      #bundle-selector .swatch-circle.color-\%ce\%bc\%cf\%89\%ce\%b2 { background: #b48fc4 !important; }
      #bundle-selector .swatch-circle.color-\%cf\%83\%ce\%ba\%ce\%bf\%cf\%8d\%cf\%81\%ce\%bf-\%ce\%bc\%cf\%80\%ce\%bb\%ce\%b5 { background: #2b3a63 !important; }
    ...
    <style>
      ...
      /* lokalizirane barve GR */
      #bundle-selector .swatch-circle.color-\%ce\%bc\%ce\%b1\%cf\%8d\%cf\%81\%ce\%bf { background: #17161a !important; }
      #bundle-selector .swatch-circle.color-\%cf\%83\%ce\%ba\%ce\%bf\%cf\%8d\%cf\%81\%ce\%bf-\%ce\%b3\%ce\%ba\%cf\%81\%ce\%b9 { background: #4a4a4f !important; }
      #bundle-selector .swatch-circle.color-\%cf\%81\%ce\%bf\%ce\%b6 { background: #e28ba4 !important; }
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 majica + 1 bokserica + 1 carapa),
        ...
        ?>
            <label style="position: relative; <?php if ( ( ( $loop_index == 1 || $loop_index == 3 ) && ! $show_group_titles ) || ( ! empty( $gck_offer_badges ) && isset( $gck_offer_badges[ $loop_index ] ) ) ) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                if (
                    ...
                    <span class="gck-offer-prices">
                        <span class="gck-per-chip">
                            ...
                            <?php endif; ?>
                            <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?>€ / τεμ.</span>
                        </span>
                    ...
                    <?php else : ?>
                        — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?>€ / τεμ.</span>
                    <?php endif; ?>
                <?php endif; ?>
                </span><!-- /.gck-offer-head -->
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes — grouped by garment word via the same helper SHBOX
                    // uses ($attr_groups). Form field names stay stable (majica_color …)
                    // so cart/label capture and the socks-width CSS are unaffected.
                    // Falls back to hardcoded options if a garment group can't be
                    // resolved, so it never breaks. Each item gets its own pair index
                    // so all 6 selections reach the cart.
                    $gck_sections = array(
                        array(
                            'label'   => 'Επίλεξε 4 μπλουζάκια',
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Μαύρο', 'Λευκό', 'Γκρι', 'Μπεζ', 'Σκούρο μπλε', 'Καφέ', 'Πράσινο' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Επίλεξε 1 μποξεράκι δώρο',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Μαύρο', 'Μπλε', 'Γκρι', 'Πράσινο', 'Κόκκινο' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Επίλεξε χρώμα καλτσών (5 ζευγάρια)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Μαύρο', 'Λευκό' ),
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Διάλεξε ' . (int) $pairs . ' ' . gck_gr_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Διάλεξε ακόμα ' . (int) $pairs . ' ' . gck_gr_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Διάλεξε <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_gr_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Διάλεξε ακόμα <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_gr_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
...
<!--
                    <small style="display: block; line-height: 1;"><?php esc_html_e( 'Προσφέρουμε 30 ημέρες επιστροφή χρημάτων ή ανταλλαγή προϊόντος – αγορές χωρίς άγχος!
 ...
 * total: majica 1-4 (colour+size), bokserica (colour+size), carapa (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Μπλουζάκι: Μαύρο - L", "Μποξεράκι: Μπλε - M",
 * "Κάλτσες: Μαύρο - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'majic' ) !== false )    { $label = 'Μπλουζάκι'; break; }
            if ( strpos( $kl, 'bokseric' ) !== false ) { $label = 'Μποξεράκι'; break; }
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Κάλτσες';   break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 majica + 1 bokserica + 1 carapa),
...
function gck_shgifts_free_gifts_notice() {
    ...
    $gifts = array(
        array( 'label' => '1x μποξεράκι', 'img' => $boxer_img, 'price' => '19,99 €' ),
    );
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x ζευγάρι κάλτσες', 'img' => $socks_img, 'price' => '12,99 €' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 ΔΩΡΕΑΝ προϊόντα σε κάθε αγορά</div>
        <div class="gck-fg__grid">
            <?php foreach ( $gifts as $g ) : ?>
                <div class="gck-fg__card">
                    <span class="gck-fg__badge">Δώρο</span>
