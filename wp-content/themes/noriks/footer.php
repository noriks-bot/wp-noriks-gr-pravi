...
<!-- suport section -->
	...
	<?php
	// ===== Klaviyo newsletter =====
	// TODO(GR): replace with the Greek footer list ID (Klaviyo -> Lists
	// & Segments -> Settings -> List ID). Left as placeholder so signups are
	// NOT sent to another market's list until the GR list is set.
	$nf_klaviyo_list = 'Szsizi';
	...
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Με επιφύλαξη παντός δικαιώματος. Οι τιμές περιλαμβάνουν ΦΠΑ. Οι εικόνες είναι ενδεικτικές.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Γίνε μέλος της οικογένειας NORIKS</h2>
					<p class="nf-signup-sub">Αποκλειστικές προσφορές και προαναγγελίες νέων προϊόντων — μάθε πρώτος.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						<label class="nf-field-label" for="nf-email">Το e-mail σου *</label>
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Η διεύθυνση email σου" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Εγγραφή</button>
						</div>
						<p class="nf-signup-note">
							Με την εγγραφή αποδέχεσαι να λαμβάνεις e-mail. Μπορείς να διαγραφείς οποιαδήποτε στιγμή.
							<a href="/gr/politiki-aporritou/">Πολιτική απορρήτου.</a>
						</p>
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/odigies' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/gr/odigies/">Οδηγίες χρήσης</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/gr.svg" alt="" class="nf-flag">
					Ελλάδα (GR)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Με επιφύλαξη παντός δικαιώματος.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Καταχώρησε μια έγκυρη διεύθυνση e-mail.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Το newsletter δεν έχει συνδεθεί ακόμη (λείπει το Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Εγγραφή…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Ευχαριστούμε! Έλεγξε το e-mail σου για επιβεβαίωση.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Κάτι πήγε στραβά, δοκίμασε ξανά.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Κάτι πήγε στραβά, δοκίμασε ξανά.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
