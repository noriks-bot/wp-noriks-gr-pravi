 ...
 *
 * Okvir se prikazuje ODMAH ISPOD gumba "Dodaj u košaricu" i nudi 4x Πράσινα μπόξερ
	...
	acf_add_local_field_group( array(
		...
		'fields' => array(
			array(
				...
				'label'        => 'Εμφάνιση upsell κάτω από το κουμπί (4x Πράσινα μπόξερ)',
				'name'         => 'noriks_pp_upsell',
				'type'         => 'true_false',
				'instructions' => 'Προσθέτει το πλαίσιο "Αγοράστε μαζί και εξοικονομήστε" ακριβώς κάτω από το κουμπί Προσθήκη στο καλάθι. Ο πελάτης επιλέγει μέγεθος και 4 τεμάχια προστίθενται στην τιμή upsell. Ισχύει μόνο για αυτό το προϊόν.',
				'ui'           => 1,
			),
			array(
				'key'          => 'field_noriks_pp_upsell2',
				'label'        => 'Εμφάνιση upsell #2 κάτω από το κουμπί (2 μπλουζάκια: μαύρο + γκρι)',
				'name'         => 'noriks_pp_upsell2',
				'type'         => 'true_false',
				'instructions' => 'Δεύτερο πλαίσιο upsell: πακέτο 2 μπλουζακιών (1 μαύρο + 1 γκρι), ο πελάτης επιλέγει ένα μέγεθος και για τα δύο. Ανεξάρτητο από τον πρώτο διακόπτη.',
				'ui'           => 1,
			),
			array(
				'key'          => 'field_noriks_pp_upsell3',
				'label'        => 'Εμφάνιση upsell #3 κάτω από το κουμπί (μπλουζάκι συμπίεσης NORIKS FIT)',
				'name'         => 'noriks_pp_upsell3',
				'type'         => 'true_false',
				'instructions' => 'Τρίτο πλαίσιο upsell: μπλουζάκι συμπίεσης NORIKS FIT, ο πελάτης επιλέγει μέγεθος. Ανεξάρτητο από τους δύο πρώτους διακόπτες.',
				'ui'           => 1,
			),
		),
		'location'   => array(
...
function noriks_pp_upsell_config() {
	return apply_filters( 'noriks_pp_upsell_config', array(
		'product_id' => 3048,                    // Zelene Bokserice (varijabilni proizvod)
		'qty'        => 4,                       // uvijek 4 komada, iste veličine
		'total'      => 19.99,                   // ista cijena kao thank you upsell (4 komada)
		'title'      => '4x Πράσινα μπόξερ',
		'desc'       => 'Αναπνέουν και είναι απαλά — προσθέστε τα στην παραγγελία με %s%% έκπτωση.', // %s = izracunati popust
		'size_attr'  => 'μέγεθος',
...
function noriks_pp_upsell_render() {
	...
	?>
	<div class="npu-wrap">
		<span class="npu-label">Αγοράστε μαζί και εξοικονομήστε:</span>
			...
			<div class="npu-grid">
				...
				<div class="npu-actions">
					<label class="npu-check">
						...
						<span class="npu-check-text">Προσθήκη στην αγορά</span>
					</label>

					<select class="npu-size" name="noriks_pp_upsell_size" aria-label="Μέγεθος μπόξερ">
...
function noriks_pp_upsell2_config() {
	return apply_filters( 'noriks_pp_upsell2_config', array(
		...
		'title'      => '1x Μαύρο + 1x Γκρι μπλουζάκι',
		'desc'       => 'Δύο βασικά μπλουζάκια σε πακέτο — προσθέστε τα στην παραγγελία με %s%% έκπτωση.',
		'size_attr'  => 'μέγεθος',
		'sku'        => 'NORIKS-SHIRTS-BLACK-GRAY-2-PACK-UPSELL',
		'image'      => get_template_directory_uri() . '/img/upsell/upsell-2x-majici.png',
		'label'      => 'Αγοράστε μαζί και εξοικονομήστε:',
		'add_text'   => 'Προσθήκη στην αγορά',
		'aria'       => 'Μέγεθος μπλουζακιών',
	) );
}
...
function noriks_pp_upsell2_maybe_add( $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data ) {
	...
	$lines  = array(
		( $prod_a ? $prod_a->get_name() : 'Μαύρο' ) . ' - ' . $size,
		( $prod_b ? $prod_b->get_name() : 'Γκρι μπλουζάκι' ) . ' - ' . $size,
	);
...
function noriks_pp_upsell3_config() {
	return apply_filters( 'noriks_pp_upsell3_config', array(
		'product_id' => 11254,   // NORIKS FIT kompresijska majica (jednostavan proizvod)
		'total'      => 17.99,  // cijena za 1 komad
		'title'      => 'Μπλουζάκι συμπίεσης NORIKS FIT',
		'desc'       => 'Διαμορφώνει τη σιλουέτα και στηρίζει τη στάση — πρόσθεσέ το με %s%% έκπτωση.',
		'size_attr'  => 'μέγεθος',
...
function noriks_pp_upsell3_order_item_meta( $item, $cart_item_key, $values, $order ) {
	if ( ! empty( $values['_noriks_pp_upsell3'] ) ) {
		...
		if ( ! empty( $values['_noriks_pp_upsell_size'] ) ) {
			$item->update_meta_data( 'μέγεθος', sanitize_text_field( $values['_noriks_pp_upsell_size'] ) );
		}
	}
}
