...
// Check if order has ONLY bokserice (no majice, no τεμplet)
...
foreach ( $order->get_items() as $item ) {
    ...
    $is_majica = ( strpos($cat_str, 'orto-majic') !== false || strpos($cat_str, 'mplouzoakia') !== false || strpos($name, 'μπλουζ') !== false || strpos($cat_str, 'majic') !== false || strpos($name, 'majic') !== false );
    ...
    // Komplet
    $is_τεμplet = ( strpos($name, 'τεμplet') !== false || strpos($cat_str, 'τεμplet') !== false );
    if ( $is_boks ) $has_bokserice = true;
    if ( $is_majica || $is_τεμplet ) $has_only_bokserice = false;
}
if ( !$has_bokserice ) $has_only_bokserice = false;

// ONLY bokserice in order → upsell MAJICE, else → upsell ΜΠΟΞΕΡΑΚΙΑ
$upsell_is_majice = $has_only_bokserice;

if ( $upsell_is_majice ) {
    $upsell_product_id = 250; // Crna majica (variable)
    $upsell_name       = 'Μαύρα Μπλουζάκια';
    $upsell_qty_prices = array( 1 => 12.99, 3 => 29.99, 6 => 39.99 );
    $upsell_qty_names  = array( 1 => '1x Μαύρο Μπλουζάκι', 3 => '3x Μαύρα Μπλουζάκια', 6 => '6x Μαύρα Μπλουζάκια' );
    $upsell_qty_images = array(
        1 => 'https://noriks.com/gr/wp-content/uploads/2025/09/black-1.jpg',
        3 => 'https://noriks.com/gr/wp-content/uploads/2025/09/black-3x.jpg',
        6 => 'https://noriks.com/gr/wp-content/uploads/2026/01/15xcrnamajica.png',
    );
    ...
    $upsell_qty_images = array(
        1 => 'https://noriks.com/gr/wp-content/uploads/2025/11/crne-boksarice-produktna.jpg',
        3 => 'https://noriks.com/gr/wp-content/uploads/2025/11/boksarice_3x_crne.png',
        5 => 'https://noriks.com/gr/wp-content/uploads/2026/01/boksarice_5x_crne.png',
    );
...
<div id="order-splash" style="position:fixed;top:0;left:0;width:100%;height:100%;background:#04ac00;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.6s ease;">
    ...
    <h1 style="color:#fff;font-family:'Roboto',sans-serif;font-size:28px;font-weight:700;margin:0;">Η παραγγελία καταχωρήθηκε!</h1>
    <p style="color:rgba(255,255,255,0.85);font-family:'Roboto',sans-serif;font-size:15px;margin:10px 0 0;">Αριθμός παραγγελίας: #<?php echo $order->get_order_number(); ?></p>
</div>
    ...
    <?php if ( $order->has_status( 'failed' ) ) : ?>
        <div class="ty-success" style="background:#fde8e8;">
            ...
            <p style="margin-top:16px;">
                <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>" style="display:inline-block;background:#E8450E;color:#fff;padding:12px 32px;text-decoration:none;font-weight:700;">Δοκιμάστε ξανά</a>
            </p>
        </div>
    <?php else : ?>
            ...
            <div class="ty_upsell_one_wrapper__popup-content">
                ...
                <div class="tyuo_timer" style="position:relative;">
                    <div class="timer_wrapper">
                        <div class="special_offer_txt">Τελευταία ευκαιρία – η προσφορά λήγει σε</div>
                ...
                <div class="tyuo_middle_section">
                    <div class="sub_title">
                        ...
                        Χωρίς επιπλέον αποστολή – όλα σε ένα πακέτο
                    </div>
                    <div class="clue_text">
                        ...
                        Συμπληρώστε τον συνδυασμό και εξοικονομήστε
                    </div>
                </div>
                ...
                <div class="tyuo_product_section">
                    ...
                    <div class="ty-qty-picker" style="display:flex;gap:8px;padding:0 0 10px;justify-content:center;">
                        ...
                        <label class="ty-qty-btn<?php echo $cls; ?>" style="flex:1;text-align:center;padding:10px 0;border:2px solid <?php echo $border; ?>;border-radius:4px;font-weight:700;font-size:14px;cursor:pointer;background:<?php echo $bg; ?>;color:#000;">
                            <input type="radio" name="ty_qty" value="<?php echo $q; ?>"<?php echo $chk; ?> style="display:none;"> <?php echo $q; ?>x τεμ
                        </label>
                        <?php endforeach; ?>
                    </div>
                        ...
                        <select class="variation-select" id="ty-variation-select">
                            <?php if ( $upsell_variations ) : ?>
                                ...
                                <option value="<?php echo $v['id']; ?>" <?php selected( strtolower($v['size']), strtolower($customer_size) ); ?>>
                                    Μαύρο, <?php echo esc_html( $v['size'] ); ?>
                                </option>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <option value="">Μαύρο, S</option>
                                <option value="">Μαύρο, M</option>
                                <option value="">Μαύρο, L</option>
                                <option value="">Μαύρο, XL</option>
                                <option value="">Μαύρο, XXL</option>
                            <?php endif; ?>
                        </select>
                    </div>
            ...
            <div class="ty-grid-popup">
                <div class="ty-grid-header" style="cursor:pointer;flex-direction:column;position:relative;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:8px;">
                        <div style="font-size:15px;font-weight:400;color:#000;">Η ειδική προσφορά λήγει</div>
                    ...
                    <div style="font-size:20px;font-weight:700;color:#000;line-height:1.3;text-align:center;padding-top:10px;">Προσθέστε οποιοδήποτε προϊόν με 50% έκπτωση</div>
                </div>
                ...
                <div class="ty-grid">
                    ...
                    <div class="ty-grid-item">
                        ...
                        <div class="g-category">ΜΠΟΞΕΡΑΚΙΑ</div>
                ...
                <div class="buttons-section">
                    <a class="pass-btn" id="ty-grid-close">Δεν θέλω</a>
                    <div class="buy-btn" id="ty-grid-add-all">ΠΡΟΣΘΗΚΗ ΣΤΗΝ ΠΑΡΑΓΓΕΛΙΑ</div>
                </div>
    ...
    if (overlay) {
        ...
        overlay.querySelectorAll('.g-select-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                if (el.classList.contains('selected')) {
                    ...
                    el.textContent = 'ΕΠΙΛΟΓΗ';
                    el.style.background = '#000';
                } else {
                    el.classList.add('selected');
                    el.innerHTML = '✔ ΕΠΙΛΕΓΜΕΝΟ <span style="font-size:10px;opacity:0.7;margin-left:4px;">αφαίρεση</span>';
                    el.style.background = '#2E7D32';
                }
            });
        });

        // "Δεν θέλω" — close all, show summary without upsells
        document.getElementById('ty-grid-close').addEventListener('click', closeAll);

        // "ΠΡΟΣΘΗΚΗ ΣΤΗΝ ΠΑΡΑΓΓΕΛΙΑ" — add all selected items, then close
        ...
        if (gridAddAll) {
            gridAddAll.addEventListener('click', function() {
                ...
                gridAddAll.textContent = 'Προσθέτω...';
    ...
    setTimeout(function() {
        ...
        if (mainBtn) {
            ...
            if (addedMap.hasOwnProperty(mainPid) || addedMap.hasOwnProperty(String(mainPid))) {
                mainBtn.textContent = '✓ ΠΡΟΣΤΕΘΗΚΕ';
